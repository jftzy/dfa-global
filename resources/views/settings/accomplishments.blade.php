<x-app-layout>
	...
	<x-slot name="header">
	    <h2 class="font-semibold text-lg text-gray-100 leading-tight">
	        {{ __('Data Settings -') }} <span class="text-[#fdc02f]">{{ __('Accomplishments') }}</span>
	    </h2>
	</x-slot>
	...
	<div x-data="options">
		...
		<div class="w-full flex flex-col p-6">
			<div class="flex flex-col items-center bg-white border border-gray-200 rounded-lg shadow md:flex-row">
			    <img class="hidden md:block object-cover rounded-t-lg h-52 w-[250px] md:rounded-none md:rounded-s-lg" src="imgs/data-analysis.jpg" alt="">
...
<script type="text/javascript">
            	...
            	{ 
            		...
            		render: function(data, type, row) {
    					return `<div style="display: flex">
	    							<a href="{{ route('settings.accomplishments-edit', '' ) }}/`+ data +`" title="EDIT">
	...
	function trashed(id) {
		...
		if (c) {
			...
			$.ajax(
		        {
		            url: "settings-accomplishment-delete/"+id,
		            type: 'POST',
		            dataType: "JSON",
		            data: {
		                "id": id,
